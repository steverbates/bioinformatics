	...
	def roc_auc(self,X=None,y_true=None,plot=True,savepath=None,sample_weight=None,drop_intermediate=True,title=None,figsize=(6.4,4.8)):
		...
		if plot:
			...
			if savepath is None:
				...
				fig.savefig(savepath)
		return area
	def save(self,savepath='grad_boos_classifier.joblib'):
		dump(self,savepath)
	def delattr(self,attributes):
		for attr in pd.Series(attributes):
			delattr(self,attr)
...
class svm_classifier:
	def __init__(self, data_frame,categ_col=None,scalar_cols=None,C=1.0,kernel='rbf',degree=3,gamma='auto_deprecated',coef0=0.0,shrinking=True,tol=0.001,cache_size=200,class_weight=None,verbose=False,max_iter=-1,decision_function_shape='ovr',random_state=None,sample_weight=None,rescale=True,pos_label=None): #data_frame rows are samples, columns are variable names.  categ_col and scalar_cols are used to set targets and input variables for model, and rescale used to indicate that each input variable will be rescaled as a fraction of its maximum value; set to False if data has already been conditioned.  pos_label can optionally be used to force the label name considered to be positive (i.e. higher probability in sigmoid output function) in two-way classification. All other parameters defaults to pass to GradientBoostingClassifier class.
		if type(data_frame) != str:
			if categ_col is None:
				self.y_train, data_frame = data_frame[data_frame.columns[-1]], data_frame.drop(columns=data_frame.columns[-1])
			elif type(categ_col) == str:
