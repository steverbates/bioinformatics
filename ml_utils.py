...
#Open question: does it make more sense to implement gradient boost as separate thing, or subclass of random forest?
...
'''
class log_reg_classifier:
	def __init__(self, df,categ_col=None,scalar_cols=None,penalty='l2', dual=False, tol=0.0001, C=1.0, fit_intercept=True, intercept_scaling=1, class_weight=None, random_state=None,solver='liblinear',max_iter=100, multi_class='ovr', verbose=0, warm_start=False,n_jobs=None,sample_weight=None,rescale=True,pos_label=None): #data_frame rows are samples, columns are variable names.  categ_col and scalar_cols are used to set targets and input variables for model, and rescale used to indicate that each input variable will be rescaled as a fraction of its maximum value; set to False if data has already been conditioned.  pos_label can optionally be used to force the label name considered to be positive (i.e. higher probability in sigmoid output function) in two-way classification. All other parameters defaults to pass to LogisticRegression class.
		if type(df) == str: #if data frame argument is a string, this is interpreted as a filepath to load a previously saved model from a file, and all other arguments are overriden.
			saved = load(df)
			for var in vars(saved).keys():
				setattr(self,var,vars(saved)[var])
		else:
			...
			if categ_col is None:
				...
				try:
					if categ_col.name is None:
						data_frame['category'] = pd.Series(categ_col,index=data_frame.index)
						categ_col = 'category'
					else:
						data_frame[categ_col.name] = pd.Series(categ_col,index=data_frame.index)
						categ_col = categ_col.name
				except AttributeError:
					data_frame['category'] = pd.Series(categ_col,index=data_frame.index)
					categ_col = 'category'
	...
	def roc_auc(self,X=None,y_true=None,plot=True,savepath=None,sample_weight=None,drop_intermediate=True,title=None,figsize=(6.4,4.8)):
		...
		if X is None:
			...
			if y_true is None:
				...
				y_score = self.predict_proba(X)['P(%s)'%(self.pos_label)]
			self.y_test = y_true
		area = roc_auc_score(y_true,y_score,average=None,sample_weight=sample_weight,max_fpr=None)
		if plot:
			fpr, tpr, thresholds = roc_curve(y_true,y_score,pos_label=self.pos_label,sample_weight=sample_weight,drop_intermediate=drop_intermediate)
...
#		return pd.DataFrame(self.model.decision_function(x[self.scalar_cols]/self.rescale_factors),index=x.index)
#	def predictors(self,x=None):
#		if x is None:
#			x = self.X
#		frame = pd.DataFrame(self.model.predict_log_proba(x[self.scalar_cols]/self.rescale_factors),index=x.index,columns=self.class_labels)
#		return frame[frame.columns[1]] - frame[frame.columns[0]]



class knn_classifier:
	def __init__(self,data_frame,categ_col=None,scalar_cols=None,n_neighbors=5, weights='uniform', algorithm='auto', leaf_size=30, p=2, metric='minkowski', metric_params=None, n_jobs=None,rescale=True,pos_label=None): #Supervised nearest neighbors model for classifying according to predetermined classes.  data_frame rows are samples, columns are variable names.  categ_col and scalar_cols are used to set targets and input variables for model, and rescale used to indicate that each input variable will be rescaled as a fraction of its maximum value; set to False if data has already been conditioned.  pos_label can optionally be used to force the label name considered to be positive in two-way classification. All other parameters defaults to pass to KNeighborsClassifier class.
		if type(data_frame) == str: #if data frame argument is a string, this is interpreted as a filepath to load a previously saved model from a file, and all other arguments are overriden.
			...
			if rescale:
				...
				self.rescale_factors = pd.DataFrame(list(self.X_train.apply(lambda x: (0,1),axis=0)),columns=['minima','ranges'])
			self.model = KNeighborsClassifier(n_neighbors=n_neighbors, weights=weights, algorithm=algorithm, leaf_size=leaf_size, p=p, metric=metric, metric_params=metric_params, n_jobs=n_jobs).fit((self.X_train - self.rescale_factors['minima'])/self.rescale_factors['ranges'],self.y_train)
			self.scalar_cols = scalar_cols
			self.categ_col = categ_col
			self.class_labels = self.model.classes_
			if len(self.class_labels) == 2: #Need to identify positive/negative labels to calculate ROC curve, sensitivity, specificity, false positive rate, positive predictive value
	...
	def confusion_matrix_plot(self,X=None,savepath=None,sample_weight=None,title=None,figsize=(6.4,4.8),cmap='hot_r'):
		if title is None:
			title = '%s %i Nearest Neighbors Model Confusion Matrix'%(self.categ_col,self.n_neighbors)
		if X is not None:
			y_true, y_pred = X[self.categ_col], self.predict(X)
			self.y_test = y_true
			cm = pd.DataFrame(confusion_matrix(y_true,y_pred,labels=self.class_labels,sample_weight=sample_weight),index=pd.Series(['%s'%s for s in self.class_labels],name='Actual'),columns=pd.Series(self.class_labels,name='Predicted')) #Setting index the same as columns, as I wanted, raises an error when feeding cm to sns.heatmap, I so had to hack it like so.
	...
	def roc_auc(self,X=None,plot=True,savepath=None,sample_weight=None,drop_intermediate=True,title=None,figsize=(6.4,4.8)):
		if title is None:
			title = '%s %i Nearest Neighbors Model ROC Curve'%(self.categ_col,self.n_neighbors)
		if len(self.class_labels)!=2:
			return None
		if X is None:
			y_true, y_score = self.y_test,self.predict_proba()['P(%s)'%(self.pos_label)]
		...
		self.roc_auc = area
		return area
	def save(self,savepath='knn_classifier.joblib'):
		dump(self,savepath)
	def delattr(self,attributes):
		for attr in pd.Series(attributes):
			delattr(self,attr)
